...
public class Termin implements Comparable<Termin>, Serializable {
    // eindeutiger Bezeichner für den Termin, wird aus anderen Attributwerten aufgebaut 
    private String tid;
    // Sinn und Zweck des Termins 
    private String text;
    // Datum der Verabredung
    private LocalDate datum;
    // Startzeitpunkt 
    private LocalTime von;
    // Endzeitpunkt 
    private LocalTime bis;
    // Kontakt der den Termin angelegt hat
    private Kontakt besitzer;
    // Teilnehmer des Termins
    private TreeSet<Kontakt> teilnehmer;        // Treeset = sortierte Menge (keine Duplikate)

    public Termin(String text, LocalDate datum, LocalTime von, LocalTime bis) throws UngueltigerTerminException {
        tid = text + datum + von + bis;     //id ist eine String konkatenation aus text/datum/von/bis
        this.text = text;
        this.datum = datum;
        this.von = von;
        this.bis = bis;
   ...
   public void addTeilnehmer(Kontakt teiln) {
    ...
    public int compareTo(Termin jenerTermin) {
        ...
        if(vergleich == 0) {
            vergleich = tid.compareTo(jenerTermin.getId());
        }
        return vergleich;
    }
